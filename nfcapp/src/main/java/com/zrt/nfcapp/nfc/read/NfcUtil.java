package com.zrt.nfcapp.nfc.read;
...
public class NfcUtil {
    ...
    public static NdefMessage[] getNdefMsg(Intent intent) {
        ...
        if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(action) ||
            ...
            if (rawMessage != null) {
                ...
                byte[] payload = dumpTagDataJson(parcelable).getBytes();
    ...
    //一般公家卡，扫描的信息
    private static String dumpTagDataJson(Parcelable p) {
        JSONObject jsonObject = new JSONObject();
        Tag tag = (Tag) p;
        byte[] id = tag.getId();
        try {
            jsonObject.put("tag_id_hex", getHex(id));
            jsonObject.put("tag_id_dec", getDec(id));
            jsonObject.put("id_reveresd", getReversed(id));
            StringBuilder sb = new StringBuilder();
            String prefix = "android.nfc.tech.";
            for (String tech : tag.getTechList()) {
                sb.append(tech.substring(prefix.length()));
            ...
            sb.delete(sb.length() - 2, sb.length());
            jsonObject.put("technologies", sb.toString());
            for (String tech : tag.getTechList()) {
                if (tech.equals(MifareClassic.class.getName())) {
                    MifareClassic mifareTag = MifareClassic.get(tag);
                    String type = "Unknown";
                    ...
                    jsonObject.put("Mifare Classic type", type);
                    jsonObject.put("Mifare size", mifareTag.getSize() + " bytes");
                    jsonObject.put("Mifare sectors", mifareTag.getSectorCount());
                    jsonObject.put("Mifare blocks", mifareTag.getBlockCount());
                }
                ...
                if (tech.equals(MifareUltralight.class.getName())) {
                    ...
                    jsonObject.put("Mifare Ultralight type", type);
                }
            }
