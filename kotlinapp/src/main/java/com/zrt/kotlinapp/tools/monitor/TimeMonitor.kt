...
object TimeMonitorConfig {
    ...
    val TIME_MONITOR_ID_ACTIVITY_START = 2
}
...
public class TimeMonitor {
     ...
     * 每打一次点，记录某个tag的耗时
     */
    fun recordingTimeTag(tag: String):TimeMonitor  {
        // 若保存过相同的tag，先清除
...
//        if(this::mTimeMonitorMap.isInitialized) // 是否已初始化
    }
    /**
     * 初始化打点模块
     */
    fun resetTimeMonitor(id: Int): TimeMonitor  {
        if (mTimeMonitorMap!![id] != null) {
            mTimeMonitorMap.remove(id)
        }
        return getTimeMonitor(id)
    }
